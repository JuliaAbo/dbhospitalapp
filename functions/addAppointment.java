...
class addAppointment implements IFunc {
  Appendable ap;
  Connection conn;
  Scanner scan;

  public addAppointment(Appendable ap, Connection conn, Scanner scan) {
    this.ap = ap;
    this.conn = conn;
    this.scan = scan;
  }
  ...
  public void apply() {
    try {
      ...
      if(new validateAppointment(this.ap, this.conn, arg1, arg3, arg4).apply("")){
        this.ap.append("This appointment already exists, try updating instead");
      }
      else {
        String call = "{CALL addAppointment(?,?,?,?,?)}";
