...
class updatePatient implements IFunc {
  Appendable ap;
  Connection conn;
  Scanner scan;

  public updatePatient(Appendable ap, Connection conn, Scanner scan) {
    this.ap = ap;
    this.conn = conn;
    this.scan = scan;
  }
  ...
  public void apply() {
    try {
      this.ap.append("You've decided to update a patient");
      this.ap.append("\n");
      ...
      if(!new validatePatientID(this.ap, this.conn).apply(arg1)){
        this.ap.append("This patient does not exist, try adding ");
      }
      else {
