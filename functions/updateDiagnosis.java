...
class updateDiagnosis implements IFunc {
  Appendable ap;
  Connection conn;
  Scanner scan;

  public updateDiagnosis(Appendable ap, Connection conn, Scanner scan) {
    this.ap = ap;
    this.conn = conn;
    this.scan = scan;
  }
  ...
  public void apply() {
    try {
      this.ap.append("You've decided to update a diagnosis");
      this.ap.append("\n");
      this.ap.append("Please enter the arguments for the diagnosis you would like to update as following and press enter:");
      this.ap.append("\n");
      ...
      if(!new validateDiagnosisID(this.ap, this.conn).apply(arg1)){
        this.ap.append("This diagnosis does not already exist.");
        ...
        String call = "{CALL updateDiagnosis(?,?,?,?,?)}";
