...
public class deleteEmployee implements IFunc {
  Appendable ap;
  Connection conn;
  Scanner scan;

  public deleteEmployee(Appendable ap, Connection conn, Scanner scan){
    this.ap = ap;
    this.conn = conn;
    this.scan = scan;
  }
  ...
  public void apply() {
    try {
      ...
      if(new validateEmployee(this.ap, this.conn).apply(arg1)) {
        String call = "{CALL deleteEmployee(?)}";
        ...
        this.ap.append("Deleted desired outcome. To see new result, type 'get_employees'");
      } else {
